		...
		<div class="container">
			...
			<div class="collapse navbar-collapse" id="ftco-nav">
				<ul class="navbar-nav mr-auto topmenu">
					...
					<li class="nav-item active"><a href="shop.php?category=0&page=1" class="nav-link">Все проекты
						</a>
					</li>
					<?php
						...
						if (isset($_GET['search'])) {
							...
							while ($row = $id->fetch_assoc()) {
								...
								while ($row = $result->fetch_assoc()) {
									...
									$result = mysqli_query($link, "SELECT `user_answer` FROM `products` WHERE `id` LIKE '$maxid'");
									while ($data = $result->fetch_assoc()) {
											...
											<div class="col-md-5 d-flex ftco-animate">
												<div class="blog-entry align-self-stretch">
													<a name="id' . $maxid . '" class="block-20 rounded"
														style="background-image: url(' . $row['product_image'] . ');background-position: bottom;height: 150px;
														width: 150px;background-size: cover;">
														</a>
														<div class="text mt-3 text-center">
														Соискатели ' . $cart_count;
									...
									while ($rowww = $resulttt->fetch_assoc()) {
										echo '<h4 class="heading">' . $rowww['product_name'] . '
